  ...
  const fakeSupabase = (table) => {
    ...
    if (table === 'model_runs') {
      return {
        insert: (row) => ({ select: () => ({ single: async () => {
          writes.runs.push(row);
          return { data: { id: 3000 + writes.runs.length }, error: null };
        }}) }),
      };
    }
    return { insert: async () => ({ error: null }) };
  };
  await snap.recordThreewayEvaluation({
    supabase: { from: fakeSupabase }, modelVersionId: 1, fixtureId: 77,
    pHome: 0.45, pDraw: 0.27, pAway: 0.28,
    bestH: { price: 2.30, bookie: 'Bet365' },
    bestD: { price: 3.50, bookie: 'Bet365' },
    bestA: { price: 3.20, bookie: 'Bet365' },
    homeEdge: 0.035, drawEdge: -0.055, awayEdge: -0.104,
    minEdge: 0.05,
    matchSignals: ['3way_ml'],
    debug: { sport: 'hockey' },
  });
  assert.strictEqual(writes.runs.length, 1);
  assert.strictEqual(writes.runs[0].market_type, 'threeway');
  assert.strictEqual(writes.cands.length, 3);
  assert.deepStrictEqual(writes.cands.map(c => c.selection_key), ['home', 'draw', 'away']);
  // home edge 3.5% < 5% min → rejected
  assert.strictEqual(writes.cands[0].passed_filters, false);
  assert.match(writes.cands[0].rejected_reason, /edge_below_min/);
});

test('recordBttsEvaluation: schrijft model_run + 2 pick_candidates (yes/no)', async () => {
  const writes = { runs: [], cands: [] };
  const fakeSupabase = (table) => {
    if (table === 'pick_candidates') {
      return { insert: async (row) => { writes.cands.push(row); return { error: null }; } };
    ...
    if (table === 'model_runs') {
      return {
        insert: (row) => ({ select: () => ({ single: async () => {
          writes.runs.push(row);
          return { data: { id: 2000 + writes.runs.length }, error: null };
        }}) }),
      };
    }
    return { insert: async () => ({ error: null }) };
  };
  await snap.recordBttsEvaluation({
    supabase: { from: fakeSupabase }, modelVersionId: 1, fixtureId: 99,
    pYes: 0.62, pNo: 0.38,
    bestYes: { price: 1.85, bookie: 'Bet365' },
    bestNo: { price: 2.20, bookie: 'Bet365' },
    yesEdge: 0.147, noEdge: -0.164, minEdge: 0.05,
    matchSignals: ['btts'],
    debug: { sport: 'football' },
  });
  assert.strictEqual(writes.runs.length, 1);
  assert.strictEqual(writes.runs[0].market_type, 'btts');
  assert.strictEqual(writes.runs[0].line, null);
  assert.strictEqual(writes.cands.length, 2);
  assert.strictEqual(writes.cands[0].selection_key, 'yes');
  assert.strictEqual(writes.cands[1].selection_key, 'no');
  assert.strictEqual(writes.cands[0].passed_filters, true);
  assert.strictEqual(writes.cands[1].passed_filters, false);
});
...
test('release metadata: app-meta en package.json voeren dezelfde versie', () => {
  assert.strictEqual(appMeta.APP_VERSION, '12.3.1');
  ...
  const mockSupabase = {
    from: () => ({
      select: () => ({
        ...
        lte: function() { return this; },
        limit: () => Promise.resolve({ data: [
          { id: 999, home_team_name: 'Fiorentina', away_team_name: 'Lecce', start_time: '2026-04-20T18:45:00Z' },
        ]}),
      }),
    }),
  };
  // Bet heeft teams in omgekeerde volgorde vs. fixture; swap-check vangt dit.
  const bet = { datum: '20-04-2026', wedstrijd: 'Lecce vs Fiorentina', sport: 'Voetbal' };
  const id = await resolveFixtureIdForBet(mockSupabase, bet);
