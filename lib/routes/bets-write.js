...
async function resolveFixtureIdForBet(supabase, bet) {
  try {
    ...
    // v12.1.6: agressievere teamnaam-normalisatie. Fixtures-tabel gebruikt
    // API-namen (bv. "ACF Fiorentina", "US Lecce"), bet.wedstrijd gebruikt
    // vaak korte versies ("Lecce", "Fiorentina"). Strip diacritics + common
    // club-prefixes/suffixes + vergelijk via word-overlap op betekenisvolle
    // tokens (len ≥ 3 na strip).
    // Alleen generieke club-code prefixes/suffixes. Echte teamnamen
    // ('Oilers', 'Bruins', etc.) NIET strippen — die zijn onderscheidend
    // tussen fixtures in dezelfde stad.
    const CLUB_STOPWORDS = new Set([
      'fc','sc','ac','as','us','acf','afc','cf','cd','ca','sv','bv','vv',
      'rc','rcd','cs','nk','hk','fk','tsv','rsc','rk','club','team',
    ]);
    ...
    const tokenize = name => {
      return stripDiacritics(String(name || '').toLowerCase())
        ...
        .split(/\s+/)
        .map(t => t.trim())
        .filter(t => t.length >= 3 && !CLUB_STOPWORDS.has(t));
    };
    const teamMatch = (a, b) => {
      if (!a || !b) return false;
      ...
      // v12.1.6: token-overlap na stopword-strip (Lecce ↔ US Lecce, Fiorentina ↔ ACF Fiorentina)
      const ta = tokenize(a);
      const tb = tokenize(b);
      if (ta.length && tb.length) {
        for (const t of ta) if (tb.includes(t)) return true;
      }
      return false;
    };
...
module.exports = function createBetsWriteRouter(deps) {
  ...
  router.get('/bets/:id/current-odds', async (req, res) => {
    try {
      ...
      const hostMap = {
        football: { host: 'v3.football.api-sports.io', path: '/odds', fixtureParam: 'fixture' },
        basketball: { host: 'v1.basketball.api-sports.io', path: '/odds', fixtureParam: 'game' },
        hockey: { host: 'v1.hockey.api-sports.io', path: '/odds', fixtureParam: 'game' },
        baseball: { host: 'v1.baseball.api-sports.io', path: '/odds', fixtureParam: 'game' },
        'american-football': { host: 'v1.american-football.api-sports.io', path: '/odds', fixtureParam: 'game' },
        handball: { host: 'v1.handball.api-sports.io', path: '/odds', fixtureParam: 'game' },
      };
      ...
      const preferred = Array.isArray(u?.settings?.preferredBookies) && u.settings.preferredBookies.length
        ...
        : ['bet365', 'unibet'];
      const oddsResp = await afGet(cfg.host, cfg.path, { [cfg.fixtureParam]: bet.fixture_id });
      ...
      for (const bk of searchBks) {
        const price = resolveOddFromBookie(bk, bet.markt || '', { matchName: bet.wedstrijd || '', sport });
        if (Number.isFinite(price) && price > best.price) {
