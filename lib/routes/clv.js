...
module.exports = function createClvRouter(deps) {
  ...
  router.post('/clv/backfill', requireAdmin, async (req, res) => {
    try {
      ...
      if (error) { console.error('[clv]', error.message); return res.status(500).json({ error: 'Interne fout · check server logs' }); }
      ...
      for (const r of candidates) {
        ...
        try {
          if (!supportsClvForBetMarkt(markt)) {
            details.push({ id, wedstrijd, sport, action: 'skip-unsupported-market', reason: 'CLV voor team totals wordt bewust niet berekend' });
            await new Promise(rs => setTimeout(rs, 75));
            continue;
          }
          ...
          let closingOdds = await fetchCurrentOdds(sport, fxId, markt, r.tip, { strictBookie: true, matchName: wedstrijd });
          ...
          if (!closingOdds && loggedOdds) {
            const snap = await fetchSnapshotClosing(supabase, { fixtureId: fxId, markt, preferredBookie: r.tip, matchName: wedstrijd });
  ...
  router.post('/clv/recompute', requireAdmin, async (req, res) => {
    try {
      ...
      for (const r of (data || [])) {
        ...
        try {
          if (!supportsClvForBetMarkt(markt)) {
            const hadStoredClv = [r.clv_pct, r.clv_odds, r.sharp_clv_pct, r.sharp_clv_odds]
              .some(v => v !== null && v !== undefined);
            if (!hadStoredClv) {
              skipped++;
              details.push({ id, wedstrijd, markt, action: 'skip-unsupported-market', reason: 'CLV voor team totals wordt bewust niet berekend' });
            } else if (dryRun) {
              updated++;
              details.push({ id, wedstrijd, markt, oldClv, newClv: null, action: 'would-clear-unsupported-market' });
            } else {
              ...
              updated++;
              details.push({ id, wedstrijd, markt, oldClv, newClv: null, action: 'cleared-unsupported-market' });
            }
            await new Promise(rs => setTimeout(rs, 75));
            continue;
          }
          ...
          const closingOdds = await fetchCurrentOdds(sport, fxId, markt, r.tip, { strictBookie: true, matchName: wedstrijd });
          if (!closingOdds || !loggedOdds) {
            failed++;
          ...
          if (fxId) {
            const mapped = marketKeyFromBetMarkt(markt, { matchName: wedstrijd });
            if (mapped) {
              try {
  ...
  router.get('/clv/backfill/probe', requireAdmin, async (req, res) => {
    try {
      ...
      if (!supportsClvForBetMarkt(markt)) {
        return res.json({
          bet: { id: betId, wedstrijd, sport, markt, loggedOdds, tip: data.tip, fixture_id: data.fixture_id,
                 datum: data.datum, tijd: data.tijd },
          diagnose: verbose,
          closingOdds: null,
          clvPct: null,
          clvSupported: false,
          unsupportedReason: 'team_total',
          rateLimit: { remaining: afRateLimit.remaining, limit: afRateLimit.limit,
                       callsToday: afRateLimit.callsToday, perSport: sportRateLimits },
        });
      }
      if (verbose.fxId) {
        closingOdds = await fetchCurrentOdds(sport, verbose.fxId, markt, data.tip, { matchName: wedstrijd });
