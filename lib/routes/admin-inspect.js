...
module.exports = function createAdminInspectRouter(deps) {
  ...
  router.get('/admin/v2/pick-distribution', requireAdmin, async (req, res) => {
    try {
      ...
      if (!list.length) {
        return res.json({ hours, total: 0, distribution: {}, note: 'geen pick_candidates in venster' });
      }
      ...
      res.json({
        ...
        distribution: dist,
        bookieSummary,
        note: 'distribution indexed by market_type → bookie → { total, accepted, rejected, byReason }',
      });
    } catch (e) {
