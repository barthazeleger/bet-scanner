...
function teamNameMatches(a, b) {
  ...
  const at = new Set(aa.split(' ').filter(t => t.length >= 3));
  const bt = bb.split(' ').filter(t => t.length >= 3);
  return bt.length > 0 && bt.some(t => at.has(t));
}

function parseTeamTotalMarket(markt, opts = {}) {
  const raw = String(markt || '');
  if (!/team\s*total|\bTT\b/i.test(raw)) return null;
  const lineMatch = raw.match(/\b(over|under)\s*(\d+\.?\d*)/i);
  if (!lineMatch) return null;
  const side = lineMatch[1].toLowerCase();
  const line = parseFloat(lineMatch[2]);
  if (!Number.isFinite(line)) return null;

  const beforeTt = raw.split(/\bTT\b|team\s*total/i)[0] || '';
  const teamLabel = beforeTt.replace(/[^\w\s.'-]/g, ' ').trim();
  const matchParts = String(opts.matchName || '').split(/\s+vs\s+/i).map(s => s.trim()).filter(Boolean);
  let team = null;

  if (/\bhome\b/i.test(raw)) team = 'home';
  if (/\baway\b/i.test(raw)) team = 'away';
  if (!team && matchParts.length >= 2) {
    if (teamNameMatches(teamLabel, matchParts[0])) team = 'home';
    else if (teamNameMatches(teamLabel, matchParts[1])) team = 'away';
  }
...
function resolveOddFromBookie(bk, markt, opts = {}) {
  ...
  const tt = parseTeamTotalMarket(markt, opts);
  if (tt) {
    if (!tt.team) return null;
    const ttBets = bets.filter(b => {
      const bn = String(b.name || '').toLowerCase();
      if (!bn.includes(`${tt.team} team total`)) return false;
      if (bn.includes('1st') || bn.includes('2nd') || bn.includes('3rd')) return false;
      if (bn.includes('period') || bn.includes('half') || bn.includes('quarter')) return false;
      return Array.isArray(b.values) && b.values.length > 0;
    });
    for (const bet of ttBets) {
      val = (bet.values || []).find(v =>
        String(v.value || '').trim().toLowerCase() === `${tt.side} ${tt.line}`.toLowerCase()
      );
      if (val) break;
    }
  }
  ...
  if (!val && ouMatch && !tt && !/1h\s|1st half|p1\s|1st period/.test(m)) {
...
function marketKeyFromBetMarkt(markt, opts = {}) {
  if (!markt) return null;
  const m = String(markt).toLowerCase();
  const tt = parseTeamTotalMarket(markt, opts);
  if (tt) {
    if (!tt.team) return null;
    return { market_type: `team_total_${tt.team}`, selection_key: tt.side, line: tt.line };
  }
