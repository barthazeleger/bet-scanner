...
function marketKeyFromBetMarkt(markt) {
  ...
  if (/over\s*\d/.test(m) && !m.includes('1h') && !m.includes('1st') && !m.includes('f5')) {
    return { market_type: 'total', selection_key: 'over' };
  }
  if (/under\s*\d/.test(m) && !m.includes('1h') && !m.includes('1st') && !m.includes('f5')) {
    return { market_type: 'total', selection_key: 'under' };
  }
  ...
  if (m.includes('f5 ') && (m.includes('over') || m.includes('under'))) {
    return { market_type: 'f5_total', selection_key: m.includes('over') ? 'over' : 'under' };
  }
  if (m.includes('f5 ')) {
    return { market_type: 'f5_ml', selection_key: m.includes('✈️') ? 'away' : 'home' };
  }
...
module.exports = { resolveOddFromBookie, marketKeyFromBetMarkt };
