...
module.exports = function createBetsData(deps) {
  ...
  async function readBets(userId = null, money = null) {
    ...
    const bets = (data || []).map(r => {
      ...
      return sanitizeUnsupportedClvFields({
        ...
        // v12.2.8 (F5): point-in-time preferred-bookie flag voor learning-loop.
        // Pre-migratie / legacy bets: kolom bestaat niet → undefined → fallback
        // naar runtime check in learning-loop.
        was_preferred_at_log_time: r.was_preferred_at_log_time,
      });
    });
  ...
  async function writeBet(bet, userId = null, unitEur = null) {
    ...
    // v12.2.8 (F5): bepaal point-in-time of de bookie preferred was. Filter
    // in updateCalibration leest later deze kolom (zie lib/learning-loop.js).
    // Zonder isPreferredBookie dep (test-modus): default true.
    const isPreferredFn = typeof deps.isPreferredBookie === 'function' ? deps.isPreferredBookie : null;
    const wasPreferred = isPreferredFn ? !!isPreferredFn(bet.tip || 'Bet365') : true;
    const basePayload = {
      ...
      user_id: userId || null,
      unit_at_time: ue,
      was_preferred_at_log_time: wasPreferred,
    };
    ...
    if (!_sequenceKnownBroken && !bet.id) {
      ...
      if (seqResult.columnMismatch) {
        // Probeer schema-tiers met sequence (no fixture_id, dan was_preferred,
        // dan unit_at_time). Schema-graceful zodat oude DB ook nog werkt.
        ...
        if (seq2.columnMismatch) {
          // v12.2.8: strip was_preferred_at_log_time (pre-migratie schema).
          const { was_preferred_at_log_time: _wp, ...noWasPref } = basePayload;
          const seq3 = await _trySequenceInsert(noWasPref, null);
          if (seq3.ok) { bet.id = seq3.betId; await maybeApplyBookieBalanceOnWrite(userId, balanceHookArgs); return; }
          if (seq3.columnMismatch) {
            const { unit_at_time, ...legacyBase } = noWasPref;
            const seq4 = await _trySequenceInsert(legacyBase, null);
            if (seq4.ok) { bet.id = seq4.betId; await maybeApplyBookieBalanceOnWrite(userId, balanceHookArgs); return; }
          }
        }
      }
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      ...
      if (err && isColumnError(err.message)) {
        // v12.2.8: strip was_preferred_at_log_time (pre-migratie schema).
        const { was_preferred_at_log_time: _wp, ...noWasPref } = base;
        err = await safeInsert(noWasPref);
        if (err && isColumnError(err.message)) {
          const { unit_at_time, ...legacy } = noWasPref;
    ...
    try {
      if (prevSettled && prevOutcome !== uitkomst) {
        await revertCalibration({
          ...
          ep: row.ep, prob: row.prob, tip: row.tip,
          was_preferred_at_log_time: row.was_preferred_at_log_time,
        }, userId);
      }
      if (newSettled) {
