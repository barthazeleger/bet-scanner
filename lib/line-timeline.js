 ...
 */
function snapshotAggregate(rowsAtT, preferredSet) {
...
function buildTimeline(rows, params = {}) {
  ...
  if (!clusters.length) {
    return {
      ...
      drift: null, preferredGap: null, stale: null, timeToMoveMs: null,
      bookmakerCountMax: 0, samples: 0,
    };
  }

  // Open = eerste cluster (chronologisch).
  const openCluster = clusters[0];
  const open = snapshotAggregate(openCluster, preferredSet);
  ...
  for (const cluster of clusters) {
    ...
    if (hasPref) {
      firstSeenOnPreferred = snapshotAggregate(cluster, preferredSet);
      break;
    }
  }
  ...
  const scanAnchor = scanAnchorCluster ? snapshotAggregate(scanAnchorCluster, preferredSet) : null;
  ...
  if (kickoffMs != null) {
    ...
    close = closeClusters.length
      ? snapshotAggregate(closeClusters[closeClusters.length - 1], preferredSet)
      : null;
    latestPreKickoff = preClusters.length
      ? snapshotAggregate(preClusters[preClusters.length - 1], preferredSet)
      : null;
  }
  ...
  const driftRef = close || latestPreKickoff || (clusters.length > 1 ? snapshotAggregate(clusters[clusters.length - 1], preferredSet) : null);
  ...
  for (let i = 1; i < clusters.length; i++) {
    const agg = snapshotAggregate(clusters[i], preferredSet);
  ...
  return {
    ...
    drift, preferredGap, stale, timeToMoveMs,
    bookmakerCountMax, samples: clusters.length,
  };
}
