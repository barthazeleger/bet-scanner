...
function computeModalAdvice(input) {
  ...
  // Damping factor: scanner paste multipliers / new-season / risk-gates
  // toe die pure Kelly NIET kent. Modal's raw edge% geeft daardoor een
  // rooskleurig getal dat niet strookt met waarom scanner origUnits koos.
  // We proxy de damping via: origUnits / pureRecFromHk(oldHk).
  // Bij Luton: pureRec(0.101) = 1.5U maar scanner gaf 0.75 → damping = 0.5.
  // Toegepast op edge: damped = pure × 0.5.
  const pureOrigRec = pureRecFromHk(oldHk);
  const dampingFactor = (origUnits > 0 && pureOrigRec > 0)
    ? Math.min(1, origUnits / pureOrigRec)
    : 1;
  const dampedEdge = edge * dampingFactor;
