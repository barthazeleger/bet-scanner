...
function assessPlayability(args = {}) {
  ...
  // Playable = execution + markt-kwaliteits-oordeel. dataRich blijft aparte
  // as — dunne enrichment maakt markt niet onspeelbaar.
  const executableBool = executable === true || (executable === null && lineQuality !== 'low');
  const playable = executableBool && lineQuality !== 'low';
  if (playable) notes.push('playable: ok');
  else if (!executableBool) notes.push('not_playable: execution blokkeert');
