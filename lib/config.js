...
const { APP_VERSION } = require('./app-meta');
...
// ── CONSTANTS ──────────────────────────────────────────────────────────────────
// v10.12.0: Telegram removed. Operator alerts route through web-push + inbox only.
const UNIT_EUR   = 25;
...
// v10.12.25 · atomic snapshot-swap: lezers krijgen frozen immutable array,
// schrijvers vervangen de hele referentie. Node.js' single-thread garandeert
// dat reference-swap atomic is, dus lezers zien ofwel de vorige ofwel de
// nieuwe array — nooit een halverwege gevulde staat. Voorkomt P0 race
// condition waar /api/picks tijdens een lange scan een gedeeltelijk gevulde
// lastPrematchPicks array kon zien.
let lastPrematchPicks = Object.freeze([]);
let lastLivePicks = Object.freeze([]);
function getLastPrematchPicks() { return lastPrematchPicks; }
function setLastPrematchPicks(v) { lastPrematchPicks = Object.freeze(Array.isArray(v) ? [...v] : []); }
function getLastLivePicks() { return lastLivePicks; }
function setLastLivePicks(v) { lastLivePicks = Object.freeze(Array.isArray(v) ? [...v] : []); }
...
// v11.3.24 Phase 7.2 · A2: PUBLIC_PATHS export hier verwijderd.
// Canonical set leeft nu uitsluitend in server.js (line ~488). Eerder was dit
// een stale kopie met /api/status nog erin, wat Codex #1 als maintainability-
// footgun flagde: een tweede bron-van-waarheid die bij een volgende refactor
// per ongeluk vertrouwd kon worden.
...
module.exports = {
  supabase, JWT_SECRET, ADMIN_EMAIL,
  // ADMIN_PASSW bewust NIET geëxporteerd (v10.10.22 fase 2 — blast radius reductie)
  VAPID_PUBLIC, VAPID_PRIVATE,
  APP_VERSION, UNIT_EUR, START_BANKROLL,
  ...
  loginCodes, ALLOWED_EXTENSIONS, ALLOWED_FILES,
