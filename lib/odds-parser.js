...
function bestFromArr(arr, options = {}) {
  ...
  // v12.1.8: `options.maxPrice` filtert kandidaten boven cap al in de
  // dedupe i.p.v. pas in de call-site check. Voorheen kon een longshot-quote
  // van een nieuw-toegevoegde bookie (bv. 888sport @ 3.80 op O/U cap 3.5)
  // de dedupe-winnaar worden → hele (markt, lijn) pick werd gedropt terwijl
  // Bet365 @ 2.10 binnen cap had gepast. Nu: filteren op maxPrice vóór
  // pickHighest, zodat binnen-cap-hoogste wint.
  ...
  const rawPool = arr || [];
  const pool = maxPrice != null
    ? rawPool.filter(o => Number.isFinite(o.price) && o.price <= maxPrice)
    : rawPool;
